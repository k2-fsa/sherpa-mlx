// sherpa-mlx/csrc/wave-reader.cc
//
...
std::vector<std::vector<float>> ReadWaveImpl(std::istream &is,
  ...
  if (header.chunk_id != 0x46464952) {
    SHERPA_MLX_LOGE("Expected chunk_id RIFF. Given: 0x%08x\n", header.chunk_id);
    *is_ok = false;
    return {};
  }
  ...
  if (header.format != 0x45564157) {
    SHERPA_MLX_LOGE("Expected format WAVE. Given: 0x%08x\n", header.format);
    *is_ok = false;
    return {};
  }
  ...
  if (header.subchunk1_id != 0x20746d66) {
    SHERPA_MLX_LOGE("Expected subchunk1_id 0x20746d66. Given: 0x%08x\n",
                    header.subchunk1_id);
    *is_ok = false;
    return {};
  }
  ...
  if (header.subchunk1_size != 16 &&
    ...
    SHERPA_MLX_LOGE("Expected subchunk1_size 16. Given: %d\n",
  ...
  if (header.audio_format != 1 && header.audio_format != 3) {
    ...
    SHERPA_MLX_LOGE("Expected audio_format 1. Given: %d\n",
                    header.audio_format);

    if (header.audio_format == static_cast<int16_t>(0xfffe)) {
      SHERPA_MLX_LOGE("We don't support WAVE_FORMAT_EXTENSIBLE files.");
    }
  ...
  if (header.byte_rate !=
    ...
    SHERPA_MLX_LOGE("Incorrect byte rate: %d. Expected: %d", header.byte_rate,
  ...
  if (header.block_align !=
    ...
    SHERPA_MLX_LOGE("Incorrect block align: %d. Expected: %d\n",
                    header.block_align,
                    (header.num_channels * header.bits_per_sample / 8));
    *is_ok = false;
    return {};
  ...
  if (header.bits_per_sample != 8 && header.bits_per_sample != 16 &&
    ...
    SHERPA_MLX_LOGE("Expected bits_per_sample 8, 16 or 32. Given: %d\n",
  ...
  *sampling_rate = header.sample_rate;
  ...
  if (header.bits_per_sample == 16 && header.audio_format == 1) {
    ...
    if (!is) {
      SHERPA_MLX_LOGE("Failed to read %d bytes", header.subchunk2_size);
      *is_ok = false;
      return {};
    }
