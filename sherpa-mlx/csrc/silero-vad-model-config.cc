// sherpa-mlx/csrc/silero-vad-model-config.cc
//
...
void SileroVadModelConfig::Register(ParseOptions *po) {
  po->Register("silero-vad-model", &model, "Path to silero VAD MLX model.");
  ...
  po->Register(
      ...
      "512, 1024, 1536 samples for 16000 sample rate Values other than these "
      "may affect model performance!");
}

bool SileroVadModelConfig::Validate() const {
  if (model.empty()) {
    SHERPA_MLX_LOGE("Please provide --silero-vad-model");
    return false;
  }

  if (!FileExists(model)) {
    SHERPA_MLX_LOGE("Silero vad model file '%s' does not exist", model.c_str());
    return false;
  }
