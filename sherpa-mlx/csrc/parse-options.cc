// sherpa-mlx/csrc/parse-options.cc
/**
...
void ParseOptions::RegisterTmpl(const std::string &name, T *ptr,
  ...
  if (other_parser_ == nullptr) {
    ...
    if (prefix_.empty()) {
      SHERPA_MLX_LOGE("Cannot use empty prefix when registering with prefix.");
      SHERPA_MLX_EXIT(-1);
    }
...
void ParseOptions::RegisterCommon(const std::string &name, T *ptr,
  ...
  if (!ptr) {
    SHERPA_MLX_LOGE("Empty pointer!");
    SHERPA_MLX_EXIT(-1);
  }
  std::string idx = name;
  NormalizeArgName(&idx);
  if (doc_map_.find(idx) != doc_map_.end()) {
    SHERPA_MLX_LOGE("Registering option twice, ignoring second time: %s",
                    name.c_str());
  } else {
...
void ParseOptions::DisableOption(const std::string &name) {
  if (argv_ != nullptr) {
    SHERPA_MLX_LOGE("DisableOption must not be called after calling Read().");
    SHERPA_MLX_EXIT(-1);
  }
  if (doc_map_.erase(name) == 0) {
    SHERPA_MLX_LOGE("Option %s was not registered so cannot be disabled: ",
                    name.c_str());
    SHERPA_MLX_EXIT(-1);
  }
...
std::string ParseOptions::GetArg(int32_t i) const {
  if (i < 1 || i > static_cast<int32_t>(positional_args_.size())) {
    SHERPA_MLX_LOGE("ParseOptions::GetArg, invalid index %d", i);
    SHERPA_MLX_EXIT(-1);
  }
...
static bool MustBeQuoted(const std::string &str, ShellType st) {
  ...
  if (st != kBash) {
    SHERPA_MLX_LOGE("Invalid shell type");
    SHERPA_MLX_EXIT(-1);
  }
...
int32_t ParseOptions::Read(int32_t argc, const char *const *argv) {
  ...
  for (i = 1; i < argc; ++i) {
    if (std::strncmp(argv[i], "--", 2) == 0) {
      ...
      if (!SetOption(key, value, has_equal_sign)) {
        PrintUsage(true);
        SHERPA_MLX_LOGE("Invalid option %s", argv[i]);
        SHERPA_MLX_EXIT(-1);
      }
    } else {
      break;
  ...
  if (print_args_) {
    ...
    strm << '\n';
    SHERPA_MLX_LOGE("%s", strm.str().c_str());
  }
  return i;
}
...
void ParseOptions::PrintConfig(std::ostream &os) const {
  ...
  for (const auto &it : doc_map_) {
    ...
    if (bool_map_.end() != bool_map_.find(key)) {
      ...
      SHERPA_MLX_LOGE("PrintConfig: unrecognized option %s [code error]",
                      key.c_str());
      SHERPA_MLX_EXIT(-1);
    }
    os << '\n';
...
void ParseOptions::ReadConfigFile(const std::string &filename) {
  ...
  if (!is.good()) {
    SHERPA_MLX_LOGE("Cannot open config file: %s", filename.c_str());
    SHERPA_MLX_EXIT(-1);
  }
  ...
  while (std::getline(is, line)) {
    ...
    if (line.substr(0, 2) != "--") {
      SHERPA_MLX_LOGE(
          ...
          "from a sherpa-mlx command-line program's config file: should "
    ...
    if (!SetOption(key, value, has_equal_sign)) {
      PrintUsage(true);
      SHERPA_MLX_LOGE("Invalid option %s in config file %s: line %d",
...
void ParseOptions::SplitLongArg(const std::string &in, std::string *key,
  ...
  if (pos == std::string::npos) {  // we allow --option for bools
    ...
    *has_equal_sign = false;
    ...
    PrintUsage(true);
    SHERPA_MLX_LOGE("Invalid option (no key): %s", in.c_str());
    SHERPA_MLX_EXIT(-1);
...
bool ParseOptions::SetOption(const std::string &key, const std::string &value,
  ...
  if (bool_map_.end() != bool_map_.find(key)) {
    if (has_equal_sign && value.empty()) {
      SHERPA_MLX_LOGE("Invalid option --%s=", key.c_str());
      SHERPA_MLX_EXIT(-1);
    }
    ...
    *(double_map_[key]) = ToDouble(value);
    ...
    if (!has_equal_sign) {
      SHERPA_MLX_LOGE("Invalid option --%s (option format is --x=y).",
                      key.c_str());
      SHERPA_MLX_EXIT(-1);
    }
...
int32_t ParseOptions::ToInt(const std::string &str) const {
  ...
  if (!ConvertStringToInteger(str, &ret)) {
    SHERPA_MLX_LOGE("Invalid integer option \"%s\"", str.c_str());
    SHERPA_MLX_EXIT(-1);
  }
  return ret;
}
...
int64_t ParseOptions::ToInt64(const std::string &str) const {
  int64_t ret = 0;
  if (!ConvertStringToInteger(str, &ret)) {
    SHERPA_MLX_LOGE("Invalid integer int64 option \"%s\"", str.c_str());
    SHERPA_MLX_EXIT(-1);
  }
  return ret;
}
...
float ParseOptions::ToFloat(const std::string &str) const {
  ...
  if (!ConvertStringToReal(str, &ret)) {
    SHERPA_MLX_LOGE("Invalid floating-point option \"%s\"", str.c_str());
    SHERPA_MLX_EXIT(-1);
  }
  return ret;
}
