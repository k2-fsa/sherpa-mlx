// sherpa-mlx/csrc/circular-buffer.cc
//
// Copyright (c)  2023-2025  Xiaomi Corporation

#include "sherpa-mlx/csrc/circular-buffer.h"
...
CircularBuffer::CircularBuffer(int32_t capacity) {
  if (capacity <= 0) {
    SHERPA_MLX_LOGE("Please specify a positive capacity. Given: %d\n",
                    capacity);
    SHERPA_MLX_EXIT(-1);
  }
...
#else
    SHERPA_MLX_LOGE("new_capacity (%d) <= original capacity (%d). Skip it.",
                    new_capacity, capacity);
#endif
    return;
  }
...
std::vector<float> CircularBuffer::Get(int32_t start_index, int32_t n) const {
  if (start_index < head_ || start_index >= tail_) {
    SHERPA_MLX_LOGE("Invalid start_index: %d. head_: %d, tail_: %d",
  ...
  int32_t size = Size();
  if (n < 0 || n > size) {
    SHERPA_MLX_LOGE("Invalid n: %d. size: %d", n, size);
    return {};
  }
  ...
  if (start_index - head_ + n > size) {
    SHERPA_MLX_LOGE("Invalid start_index: %d and n: %d. head_: %d, size: %d",
