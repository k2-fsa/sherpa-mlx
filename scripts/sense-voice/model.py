...
class ConstantPad1d(nn.Module):
    ...
    def __call__(self, x):
        # Assumes x shape: (B, T, C)
        left, right = self.pad
        pad = [(0, 0), (left, right), (0, 0)]  # Only pad the (T) dimension
        return mx.pad(x, pad, mode="constant", constant_values=self.value)
...
class SinusoidalPositionEncoder(nn.Module):
    ...
    def encode(
        ...
        log_timescale_increment = mx.log(mx.array([10000], dtype=dtype)) / (
            depth / 2 - 1
        )
        inv_timescales = mx.exp(
            mx.arange(depth / 2, dtype=dtype) * (-log_timescale_increment)
        )
        inv_timescales = mx.reshape(inv_timescales, [batch_size, -1])
        scaled_time = mx.reshape(positions, [1, -1, 1]) * mx.reshape(
    ...
    def __call__(self, x):
        ...
        positions = mx.arange(1, timesteps + 1)[None, :]
...
class MultiHeadedAttentionSANM(nn.Module):
    ...
    def __init__(
        ...
        self.pad_fn = ConstantPad1d(padding=(left_padding, right_padding), value=0.0)
    ...
    def forward_qkv(self, x):
        ...
        q_h = mx.reshape(q, (b, t, self.h, self.d_k)).swapaxes(
            1, 2
        )  # (batch, head, time1, d_k)
        k_h = mx.reshape(k, (b, t, self.h, self.d_k)).swapaxes(
            1, 2
        )  # (batch, head, time2, d_k)
        v_h = mx.reshape(v, (b, t, self.h, self.d_k)).swapaxes(
            1, 2
        )  # (batch, head, time2, d_k)
    ...
    def forward_attention(self, value, scores, mask, mask_att_chunk_encoder=None):
        ...
        if mask is not None:
            ...
            mask = mx.equal(mx.expand_dims(mask, axis=1), 0)  # (batch, 1, *, time2)

            min_value = -float(
                "inf"
            )  # float(numpy.finfo(torch.tensor(0, dtype=scores.dtype).numpy().dtype).min)
            #  scores = scores.masked_fill(mask, min_value)
            scores = mx.where(mask, min_value, scores)
            attn = mx.softmax(scores, axis=-1)
            attn = mx.where(mask, 0.0, attn)  # (batch, head, time1, time2)
        else:
            attn = mx.softmax(scores, axis=-1)  # (batch, head, time1, time2)

        p_attn = self.dropout(attn)
        x = mx.matmul(p_attn, value)  # (batch, head, time1, d_k)
        x = x.swapaxes(1, 2).reshape(
    ...
    def forward(self, x, mask, mask_shfit_chunk=None, mask_att_chunk_encoder=None):
        ...
        scores = mx.matmul(q_h, k_h.swapaxes(2, 3))
...
def sequence_mask(lengths, maxlen=None, dtype=mx.float32):
    if maxlen is None:
        maxlen = lengths.max()
    row_vector = mx.arange(0, maxlen, 1)
    matrix = mx.expand_dims(lengths, axis=-1)
...
class SenseVoiceEncoderSmall(nn.Module):
    def __init__(self):
        ...
        self.after_norm = nn.LayerNorm(self.output_size)

        self.tp_norm = nn.LayerNorm(self.output_size)
    ...
    def forward(
        ...
        """
        Args:
          xs_pad: (N, T, C), e.g., (1, 101, 560)
        """
        maxlen = xs_pad.shape[1]
        masks = sequence_mask(ilens, maxlen=maxlen)[:, None, :]
        ...
        # forward encoder2
        olens = masks.squeeze(1).sum(1).astype(mx.int32)
...
class CTC(nn.Module):
    ...
    def softmax(self, hs_pad):
        ...
        if self.ctc_lo is not None:
            return nn.softmax(self.ctc_lo(hs_pad), axis=2)
        else:
            return nn.softmax(hs_pad, axis=2)
...
class SenseVoiceSmall(nn.Module):
    ...
    def forward(self, x, x_len, language, text_norm):
        language_query = mx.tile(self.embed(language), (x.shape[0], 1, 1))

        textnorm_query = mx.tile(self.embed(text_norm), (x.shape[0], 1, 1))

        x = mx.concatenate((textnorm_query, x), axis=1)
        x_len += 1

        event_emo_query = mx.tile(self.embed(mx.array([[1, 2]])), (x.shape[0], 1, 1))
        input_query = mx.concatenate((language_query, event_emo_query), axis=1)

        x = mx.concatenate((input_query, x), axis=1)
        x_len += 3
